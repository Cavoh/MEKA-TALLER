...
import { AlertTriangle, Calendar, TrendingUp, Filter, Settings, ArrowUpRight } from 'lucide-react';
...
const CriticalStockTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white p-4 rounded-2xl shadow-2xl border border-gray-100 min-w-[200px] z-50">
        <p className="font-black text-[11px] uppercase text-gray-800 mb-2 border-b border-gray-50 pb-2">{label}</p>
        <div className="space-y-1.5">
          <p className="text-[10px] font-bold text-gray-500">
            Stock Actual: <span className="text-red-600 font-black">{data.stock} unidades</span>
          </p>
          <p className="text-[10px] font-bold text-gray-400">
            Stock Mínimo: <span className="text-gray-600 font-black">{data.minStock} unidades</span>
          </p>
        </div>
      </div>
    );
...
export const CriticalStockChart = ({ data }: { data: any[] }) => (
  <div className="bg-[var(--modal-bg)] p-3 md:p-4 rounded-[1.5rem] card-shadow border border-[var(--border-main)]/30 group transition-all duration-300 hover:shadow-md">
...
const DistributionTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white p-4 rounded-2xl shadow-2xl border border-gray-100 min-w-[180px] z-50">
        <p className="font-black text-[11px] uppercase text-gray-800 mb-1.5 border-b border-gray-50 pb-1.5">{name}</p>
...
export const DailyRevenueChart = ({ data }: { data: any[] }) => (
  <div className="bg-[var(--modal-bg)] p-3 md:p-4 rounded-[1.5rem] card-shadow border border-[var(--border-main)]/30 group transition-all duration-300 hover:shadow-md md:col-span-2">
    <div className="flex items-center gap-2 mb-2">
    ...
    <div className="h-[180px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} layout="vertical" margin={{ left: 50 }}>
          ...
          <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fontSize: 9, fontStyle: 'italic', fontWeight: 900, fill: '#1e293b' }} width={150} />
          <Tooltip cursor={{ fill: '#f0fdf4' }} contentStyle={{ borderRadius: '20px', border: 'none', boxShadow: '0 20px 25px -5px rgb(0 0 0 / 0.1)' }} formatter={(val: number) => [val, 'Unidades Vendidas']} />
    ...
    <div className="h-[180px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ bottom: 20 }}>
          ...
          <Tooltip cursor={{ fill: '#f5f3ff' }} contentStyle={{ borderRadius: '20px', border: 'none', boxShadow: '0 20px 25px -5px rgb(0 0 0 / 0.1)' }} formatter={(val: number) => [`$${val.toLocaleString()}`, 'Ventas']} />
